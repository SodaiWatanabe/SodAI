...
class InferenceWorker:
    ...
    async def _process(self, message_id: str, fields: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.exception("Inference transport failed for run %s", job.run_id)
    ...
    async def _generate(self, job: GenerationJob, *, resume_after_sequence: int = -1) -> None:
        ...
        try:
            ...
            logger.exception("Hina prompt construction failed for run %s", job.run_id)
            await emit(self._failed_event(job, sequence))
            return
        ...
        while True:
            try:
                ...
                logger.error("Hina generation ended without a terminal step for run %s", job.run_id)
                await emit(self._failed_event(job, sequence + 1))
                return
            except Exception:
                logger.exception("Hina generation failed for run %s", job.run_id)
                await emit(self._failed_event(job, sequence + 1))
                return
