...
@pytest.mark.parametrize("answerer_id", [AnswererId.ASUKA_1, AnswererId.ASUKA_1_1])
def test_guest_cannot_select_asuka(answerer_id: AnswererId) -> None:
    with pytest.raises(AnswererAccessError):
        ThreadService.select_answerer(principal(PrincipalKind.GUEST), answerer_id)


def test_authenticated_account_can_select_asuka_1_1() -> None:
    answerer = ThreadService.select_answerer(
        principal(PrincipalKind.USER), AnswererId.ASUKA_1_1
    )

    assert answerer.runtime_kind is RuntimeKind.LOCAL_MODEL
    assert answerer.runtime_name == "asuka-1"
    assert answerer.deployment_name == "asuka-1.1"
    assert answerer.generation_temperature == 0.85
    assert answerer.generation_max_output_tokens == 256


def test_asuka_1_is_no_longer_selectable() -> None:
    with pytest.raises(AnswererAccessError):
        ThreadService.select_answerer(principal(PrincipalKind.USER), AnswererId.ASUKA_1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (PrincipalKind.USER, AnswererId.ASUKA_1_1),
    ],
)
...
def test_answerer_catalog_is_the_single_ui_source() -> None:
    ...
    assert [(item.id, item.name, item.description, item.is_default) for item in answerers] == [
        (AnswererId.ASUKA_1_1, "Asuka 1.1", "会話に最適。", True),
    ...
    assert AnswererId.ASUKA_1 not in by_id
    assert by_id[AnswererId.ASUKA_1_1].pricing.kind is AnswererPricingKind.METERED
    assert (
        by_id[AnswererId.ASUKA_1_1].pricing.tariff_revision,
        by_id[AnswererId.ASUKA_1_1].pricing.fixed_charge,
        by_id[AnswererId.ASUKA_1_1].pricing.input_token_rate,
        by_id[AnswererId.ASUKA_1_1].pricing.output_token_rate,
        by_id[AnswererId.ASUKA_1_1].pricing.maximum_charge,
        by_id[AnswererId.ASUKA_1_1].pricing.unmetered_charge,
    ) == ("asuka-1.1-flat-v1", 100_000, 0, 0, 100_000, 100_000)
    ...
    assert by_id[AnswererId.ASUKA_1_1].is_legacy is False
    ...
    assert [option.id for option in by_id[AnswererId.ASUKA_1_1].reasoning_efforts] == [
