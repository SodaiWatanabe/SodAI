"""expire newly earned credits after 90 days
...
def _replace_lot_guard(*, earned_expiration_required: bool) -> None:
    earned_expiration_mismatch = (
        "lot_expires_at IS NULL"
        if earned_expiration_required
        else "lot_expires_at IS NOT NULL"
    )
    op.execute(
        f"""
...
def upgrade() -> None:
    op.execute("ALTER TABLE app.credit_lots DISABLE TRIGGER credit_lots_immutable")
    op.execute(
        """
        UPDATE app.credit_lots
        SET expires_at = issued_at + interval '2160 hours'
        WHERE source_kind = 'earned'
        """
    )
    op.execute("ALTER TABLE app.credit_lots ENABLE TRIGGER credit_lots_immutable")
    _replace_lot_guard(earned_expiration_required=True)
    op.create_check_constraint(
        op.f("ck_credit_lots_earned_expiration"),
        "credit_lots",
...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE app.credit_lots ENABLE TRIGGER credit_lots_immutable")
    _replace_lot_guard(earned_expiration_required=False)
